...
object HTMaterialLibraryBlock :
    ...
    override fun onUse(
        ...
        if (hand == Hand.MAIN_HAND) {
            ...
            val unifiedStack: ItemStack = HTApiHolder.Material.apiInstance.materialItemManager.convert(stack)
            val direction: Direction = state.get(Properties.HORIZONTAL_FACING)
            dropUnifiedStack(world, pos, direction, unifiedStack)
    ...
    override fun getInventory(state: BlockState, world: WorldAccess, pos: BlockPos): SidedInventory =
        LibraryInventory(world, pos, state.get(Properties.HORIZONTAL_FACING))

    private class LibraryInventory(val world: WorldAccess, val pos: BlockPos, val direction: Direction) :
        SidedInventory {
        ...
        override fun setStack(slot: Int, stack: ItemStack) {
            val unifiedStack: ItemStack = HTApiHolder.Material.apiInstance.materialItemManager.convert(stack)
            (world as? World)?.run {
                dropUnifiedStack(this, pos, direction, unifiedStack)
            }
        }
